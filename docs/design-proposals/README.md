...
| `SoftGlass.dc.html` + `SoftGlassPulse.dc.html` | D · Soft Glass (2026-09-26) | Frosted-glass window on a grey gradient, sidebar + survey drawer with stacked cards, pill statuses, one warm accent (Outfit); org-level numbers are the real Moorbach values after D4.8, department rows and deltas are illustrative. Second artboard: the pulse on a phone in the same look. Canvas: https://claude.ai/artifact/2UPA67HrKysPp9LWY3HjzP |
